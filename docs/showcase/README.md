...
- React Native one-command proof (`pnpm run test:react-native-one-command` + `pnpm run validate:react-native-one-command`): verifies `react-native-one-command/v2` preserves proof labels and exposes an optional live bridge into mobile-change verification/intake. The committed fixture remains blocked before live.
