...
- [ ] 新增 `packages/core/src/session-scheduler.ts`
  - [ ] 提供 `runExclusive(sessionId, deviceId, toolName, task)`
  - [ ] 进入 task 前将 lease 标记为 `busy`
  - [ ] task 完成后标记回 `leased/idle`
  - [ ] task 抛错也必须恢复状态（finally）

- [ ] 在 `execution-coordinator.ts` 增加
  - [ ] `markBusy`
  - [ ] `markIdle`
  - [ ] `refreshHeartbeat`
  - [ ] `recoverStaleLeases(ttlMs)`

---

## B.2 `mcp-server` 统一接入包装器

- [ ] 修改 `packages/mcp-server/src/index.ts`
  - [ ] 新增 `withSessionExecution`（与 `withPolicy`、`withPolicyAndAudit` 串联）
  - [ ] 仅对 session-bound 工具启用（见下文清单）
...
- [ ] 在工具入口（建议 start_session 前或独立定时触发）调用 `recoverStaleLeases`
- [ ] stale 判定：`heartbeatAt + ttlMs < now`
- [ ] 回收时追加 timeline event：`lease_recovered_stale`
- [ ] 回收后允许新 session 获取租约
