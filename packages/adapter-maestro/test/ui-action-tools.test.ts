...
test("verifyResolvedIosPoint finds target node at resolved point, not Application root", async () => {
	...
	assert.equal(
		...
		true,
		"Should find Button at resolved point, not use Application root",
	);
				...
				stdout: JSON.stringify([
					{
						...
						AXLabel: "Email",
						frame: { x: 40, y: 220, width: 280, height: 60 },
					},
				]),
				stderr: "",
			},
				...
				stdout: JSON.stringify([
					{
						...
						AXLabel: "Password",
						frame: { x: 40, y: 300, width: 280, height: 60 },
					},
				]),
				stderr: "",
			},
...
test("verifyTypedIosPostconditionWithHooks finds editable node at resolved point, not root", async () => {
	...
	assert.equal(
		verification.verified,
		true,
		"Should find TextField at resolved point, not use Application root",
	);
	assert.equal(verification.reasonCode, REASON_CODES.ok);
});
...
test("isIosSimulatorOnlyIdbActionError detects simulator-lifecycle protocol failures", () => {
	...
	assert.equal(
		isIosSimulatorOnlyIdbActionError("some generic idb failure"),
		false,
	);
});
...
test("tapWithMaestroTool requires structured owned-runner result for local manual runner", async () => {
	const previousBackend = process.env.IOS_PHYSICAL_ACTION_BACKEND;
	...
	try {
		process.env.IOS_PHYSICAL_ACTION_BACKEND = "local_manual_runner";
		const result = await tapWithMaestroTool({
			sessionId: "ios-physical-tap-local-structured",
			platform: "ios",
			deviceId: "00008101-000D482C1E78001E",
			x: 100,
			y: 220,
...
test("typeTextWithMaestroTool maps structured owned-runner failure to failed status", async () => {
	const previousBackend = process.env.IOS_PHYSICAL_ACTION_BACKEND;
	setExecuteRunnerForTesting(async () => ({
		exitCode: 0,
		stdout: `${OWNED_RUNNER_RESULT_PREFIX}{"commandId":"ios-physical-type-local-structured-type_text","status":"failed","reasonCode":"ACTION_TYPE_FAILED","durationMs":4,"data":{},"artifacts":[],"message":"native typing failed"}`,
		stderr: "",
	}));
	try {
		process.env.IOS_PHYSICAL_ACTION_BACKEND = "local_manual_runner";
		const result = await typeTextWithMaestroTool({
			sessionId: "ios-physical-type-local-structured",
			platform: "ios",
			deviceId: "00008101-000D482C1E78001E",
			text: "hello",
			dryRun: false,
...
test("buildIosPhysicalActionExecutionPlan builds local manual runner command when configured", () => {
	...
	assert.equal(
		plan.envPatch.IOS_OWNED_RUNNER_UDID,
		"00008101-000D482C1E78001E",
	);
	assert.equal(
...
test("classifyIosPhysicalStartupFailure classifies invalid-signature install failures", () => {
	const classified = uiActionToolInternals.classifyIosPhysicalStartupFailure({
		stderr:
			"Failed to verify code signature of OwnedRunnerUITests-Runner.app : 0xe8008018 (The identity used to sign the executable is no longer valid.)",
		exitCode: 13,
	});
...
test("classifyIosPhysicalStartupFailure does not over-classify generic install failures as signature issues", () => {
	const classified = uiActionToolInternals.classifyIosPhysicalStartupFailure({
		stderr:
			"Unable to install test runner on device due to transient install service error",
		exitCode: 13,
	});
...
test("buildIosPhysicalExecutionEvidencePaths keeps session-scoped deterministic artifact path", () => {
	...
	assert.equal(
		paths.relativePath,
		"output/evidence/ios-physical-actions/session-xyz/tap.execution.md",
	);
});
...
test("buildOwnedRunnerActionEnv ignores wildcard flow appId and preserves action payload", () => {
	const envPatch = buildOwnedRunnerActionEnv({
		actionType: "tap",
		flowContent: ['appId: "*"', "---", "- tapOn:", '    point: "12,34"'].join(
			"\n",
		),
	});
