...
- `validate:mobile-change-repo-app-success-candidate` is an offline drift check for the repo-owned demo Android app success candidate. It verifies the deterministic contract and reuses the committed candidate's artifact availability so clean CI does not require an ignored local APK build output, while keeping no-device output blocked until a real or explicitly labeled emulator run passes intake.
...
- `validate:mobile-change-failure-memory` is an offline drift check for deterministic failure-pattern grouping and bounded next-action routing. It groups repeated app-readiness failures, environment blockers, and network policy evidence without claiming autonomous root-cause certainty.
...
- Real-device confidence should be validated through Explorer/probe artifacts first, then historical showcase assets when reviewing demos:
  ...
  - `docs/showcase/evidence/mobile-change-failure-memory/` plus `pnpm run validate:mobile-change-failure-memory` for deterministic repeated-failure routing and bounded remediation recommendations
