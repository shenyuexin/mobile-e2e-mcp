...
import type { WdaRealDeviceBackend } from "./ios-backend-wda.js";
import { transformWdaElement } from "./ios-backend-wda.js";
import {
...
type WdaElementInput = Parameters<typeof transformWdaElement>[0];

function readWdaResponseValue(data: unknown): unknown {
...
interface RunUiWaitPollingLoopOptions {
	...
	captureSnapshot: () => Promise<
		UiRuntimeSnapshot | AndroidUiSnapshotFailure | IosUiSnapshotFailure
	>;
...
function normalizeAndroidUiSnapshot(
...
function shouldContinueScrollResolution(
	status: UiTargetResolution["status"],
): boolean {
...
export function buildIdbCommand(baseArgs: string[]): string[] {
	...
	return companionPath
		? [idbCliPath, "--companion-path", companionPath, ...baseArgs]
		: [idbCliPath, ...baseArgs];
}

const DEFAULT_UI_ACTION_TIMEOUT_MS = 30_000;

export async function probeIdbAvailability(
	repoRoot: string,
): Promise<CommandExecution | undefined> {
	return executeRunner(
		buildIdbCommand(["--help"]),
		repoRoot,
		process.env,
	).catch(() => undefined);
}
...
export async function executeUiActionCommand(options: {
	...
	if (options.requiresProbe) {
		result.probeExecution = await options.probeRuntimeAvailability?.(
			options.repoRoot,
		);
	...
	if (options.command[0] === "__wda_http__") {
		const [, _deviceId, method, wdaPath, body] = options.command;
		...
		if (
			...
			result.execution = {
				exitCode: 1,
				stdout: "",
				stderr: `WDA_BASE_URL must be http://localhost or http://127.0.0.1, got: ${baseUrl}`,
			};
			return result;
		}
		...
		try {
			...
			if (!response.ok) {
				result.execution = {
					exitCode: 1,
					stdout: "",
					stderr: `WDA ${method} ${wdaPath} returned ${response.status}`,
				};
			} else {
				const data = (await response.json()) as unknown;
				const rawValue = readWdaResponseValue(data);
				...
				if (wdaPath === "/source" && rawValue && typeof rawValue === "object") {
					const transformed = transformWdaElement(rawValue as WdaElementInput);
			...
			result.execution = {
				exitCode: 1,
				stdout: "",
				stderr: `WDA request failed: ${error instanceof Error ? error.message : String(error)}`,
			};
		}
		return result;
	}

	if (options.command[0] === "__owned_runner_unsupported__") {
		result.execution = {
			exitCode: 1,
			stdout: "",
			stderr: `Owned runner backend does not yet support ${options.command[2] ?? "unknown"}. Use IOS_EXECUTION_BACKEND=wda for this surface until owned runner support lands.`,
		};
		return result;
	}

	if (options.command[0] === "__owned_runner_hierarchy__") {
		const [, deviceId] = options.command;
		const commandId = `${deviceId}-hierarchy`;
		const ownedRunnerCommand = {
			commandId,
			sessionId: commandId,
			action: "hierarchy" as const,
		};
		const execution = await executeRunnerWithTestHooks(
			["bash", "scripts/dev/run-ios-owned-physical-runner.sh", "execute-flow"],
			options.repoRoot,
			{
				...process.env,
				IOS_OWNED_RUNNER_UDID: deviceId,
				IOS_OWNED_RUNNER_FLOW_PATH: "owned-runner-hierarchy",
		...
		const runnerResult = parseOwnedRunnerResultFromStdout(execution.stdout);
		if (execution.exitCode !== 0) {
			result.execution = {
				...
				stderr:
					...
					`Owned runner hierarchy transport failed with exitCode ${execution.exitCode}.`,
			};
			return result;
		}
		if (!runnerResult) {
			result.execution = {
				...
				stdout: "",
				stderr:
					"Owned runner hierarchy capture completed without a structured owned-runner result.",
			};
			return result;
		}
		if (runnerResult.commandId !== commandId) {
			result.execution = {
				exitCode: 1,
				stdout: "",
				stderr: `Owned runner hierarchy result commandId mismatch: expected ${commandId}, got ${runnerResult.commandId}.`,
			};
			return result;
		}
		const hierarchyJson = runnerResult.data.hierarchyJson;
		if (
			runnerResult.status !== "success" ||
			runnerResult.reasonCode !== REASON_CODES.ok ||
			typeof hierarchyJson !== "string"
		) {
			result.execution = {
				exitCode: 1,
				stdout: "",
				stderr:
					runnerResult.message ??
					"Owned runner hierarchy capture did not return hierarchyJson with OK reasonCode.",
			};
			return result;
		}
		const nodes = parseIosInspectNodes(hierarchyJson);
		if (nodes.length === 0) {
			result.execution = {
				exitCode: 1,
				stdout: "",
				stderr:
					"Owned runner hierarchy capture returned empty or unparseable hierarchyJson.",
			};
			return result;
		}
		result.execution = { exitCode: 0, stdout: hierarchyJson, stderr: "" };
		return result;
	}

	result.execution = await executeRunnerWithTestHooks(
		options.command,
		options.repoRoot,
		process.env,
		{
...
export function buildIosUiDescribePointCommand(
	deviceId: string,
	x: number,
	y: number,
): string[] {
...
export function buildIosSwipeCommand(
	deviceId: string,
	swipe: {
...
export function isAndroidUiSnapshotFailure(
	value: AndroidUiSnapshot | AndroidUiSnapshotFailure,
): value is AndroidUiSnapshotFailure {
	return "message" in value;
}

export function isIosUiSnapshotFailure(
	value: IosUiSnapshot | IosUiSnapshotFailure,
): value is IosUiSnapshotFailure {
	return "message" in value;
}

export async function captureAndroidUiSnapshot(
	repoRoot: string,
	deviceId: string,
	sessionId: string,
	...
	const readProducedValidXml =
		readExecution.exitCode === 0 &&
		readExecution.stdout.trim().startsWith("<?xml");
	...
	if (dumpExecution.exitCode !== 0 && !readProducedValidXml) {
		return {
			...
			command,
			message:
				"Check Android device state and ensure uiautomator dump is permitted before retrying UI resolution.",
		};
	}
	...
	const nodes =
		readExecution.exitCode === 0
			...
			: [];
	const summary =
		readExecution.exitCode === 0 ? buildInspectUiSummary(nodes) : undefined;
	const queryResult =
		readExecution.exitCode === 0
...
export async function captureIosUiSnapshot(
	repoRoot: string,
	deviceId: string,
	sessionId: string,
	runnerProfile: string,
	...
	if (backend.backendId === "axe") {
		...
		const result = await wdaBackend.executeWdaRequest(
			deviceId,
			"GET",
			"/source",
		);
		if (result.success) {
			const transformed = wdaBackend.transformWdaSource(
				result.data as WdaElementInput,
			);
			execution = {
				exitCode: 0,
				stdout: JSON.stringify(transformed),
			...
			execution = {
				exitCode: 1,
				stdout: "",
				stderr: result.error ?? "WDA request failed",
			};
		}
	} else if (backend.backendId === "owned-runner") {
		command = backend.buildHierarchyCaptureCommand(deviceId);
		...
		execution = actionResult.execution ?? {
			...
			stdout: "",
			stderr:
				"Owned runner hierarchy capture did not produce an execution result.",
		};
		...
		if (!idbProbe || idbProbe.exitCode !== 0) {
			return {
				...
				command,
				message:
					"iOS hierarchy capture failed. For simulators, install axe (brew install cameroncooke/axe/axe). For physical devices, set up WDA and run iproxy 8100 8100 --udid <udid>.",
			};
		}
		execution = await executeRunner(command, repoRoot, process.env);
	} else {
		...
		return {
			...
			outputPath: relativeOutputPath,
			command: [],
			message: `Backend ${backend.backendId} does not support hierarchy capture. Use axe (simulators) or WDA (physical devices).`,
		};
	}
	...
	if (execution.exitCode !== 0 && backend.backendId !== "idb") {
		return {
			...
			outputPath: relativeOutputPath,
			command,
			message: `iOS hierarchy capture via ${backend.backendName} failed: ${execution.stderr.trim()}`,
		};
	}
	...
	const nodes =
		execution.exitCode === 0 ? parseIosInspectNodes(execution.stdout) : [];
	if (execution.exitCode === 0 && isDegenerateIosSnapshot(nodes)) {
		return {
			...
			command,
			message:
				"iOS hierarchy capture returned only a degenerate application root. The app may not be inspectable yet; retry after launch settles or verify the current simulator UI state.",
		};
	}
	const summary =
		execution.exitCode === 0 ? buildInspectUiSummary(nodes) : undefined;
	const queryResult =
...
export async function captureIosUiRuntimeSnapshot(
	...
	return isIosUiSnapshotFailure(snapshot)
		? snapshot
		: normalizeIosUiSnapshot(snapshot);
}
...
export async function runUiWaitPollingLoop(
	...
	let lastCaptureFailure:
		| AndroidUiSnapshotFailure
		| IosUiSnapshotFailure
		| undefined;
			...
			if (retryableFailure) {
				...
				if (
					...
					return {
						...
						state: buildWaitPollingFailureStateFromSnapshot(
							options.query,
							capture,
						),
					};
				...
				if (
					isWaitConditionMet(
						{ query: options.query, ...capture.queryResult },
						options.waitUntil,
					)
				) {
					return {
	...
	if (lastCaptureFailure) {
		return {
			...
			state: buildWaitPollingStateFromFailure(
				options.query,
				lastCaptureFailure,
			),
		};
...
export async function runUiScrollResolveLoop(
	...
	while (swipesPerformed <= options.maxSwipes) {
		...
		if (!shouldContinueScrollResolution(snapshotState.resolution.status)) {
			return {
				outcome:
					snapshotState.resolution.status === "resolved"
						? "resolved"
						: "stopped",
				state: snapshotState,
			};
		...
		if (swipesPerformed === options.maxSwipes) {
			...
			if (!isUiRuntimeSnapshotFailure(finalCapture)) {
				const finalRetryableFailure =
					options.buildRetryableSnapshotFailure?.(finalCapture);
				if (!finalRetryableFailure) {
					...
					if (
						!shouldContinueScrollResolution(
							finalSnapshotState.resolution.status,
						)
					) {
						return {
							outcome:
								finalSnapshotState.resolution.status === "resolved"
									? "resolved"
									: "stopped",
