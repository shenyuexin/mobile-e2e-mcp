...
export function buildResolutionNextSuggestions(status: "resolved" | "no_match" | "ambiguous" | "missing_bounds" | "disabled_match" | "off_screen" | "unsupported" | "not_executed", toolName: string): string[] {
  ...
  if (status === "off_screen") return [`A matching UI node was found for ${toolName}, but it is currently outside the visible viewport. Scroll toward the candidate before retrying.`];
