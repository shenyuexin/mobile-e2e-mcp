...
export function buildRecordSessionRelativePath(
	recordSessionId: string,
): string {
...
export function buildRecordEventsRelativePath(recordSessionId: string): string {
	...
	return path.posix.join(
		"artifacts",
		"record-events",
		`${recordSessionId}.jsonl`,
	);
}

export function buildRecordedStepsRelativePath(
	recordSessionId: string,
): string {
	assertSafeSessionId(recordSessionId);
	return path.posix.join(
...
function buildSessionRecordAbsolutePath(
	repoRoot: string,
	sessionId: string,
): string {
...
function buildActionRecordAbsolutePath(
	repoRoot: string,
	actionId: string,
): string {
...
function buildRecordSessionAbsolutePath(
	repoRoot: string,
	recordSessionId: string,
): string {
	return path.resolve(
		repoRoot,
		buildRecordSessionRelativePath(recordSessionId),
	);
}

function buildRecordEventsAbsolutePath(
	repoRoot: string,
	recordSessionId: string,
): string {
...
function buildRecordedStepsAbsolutePath(
	repoRoot: string,
	recordSessionId: string,
): string {
	return path.resolve(
		repoRoot,
		buildRecordedStepsRelativePath(recordSessionId),
	);
}

function buildSessionAuditAbsolutePath(
	repoRoot: string,
	sessionId: string,
): string {
...
function buildAiFirstIndexAbsolutePath(
	repoRoot: string,
	fileName: string,
): string {
...
async function writeRecordSession(
	...
	const absolutePath = buildRecordSessionAbsolutePath(
		repoRoot,
		recordSessionId,
	);
	await writeJsonFile(absolutePath, record);
...
async function writeRecordedSteps(
	...
	const absolutePath = buildRecordedStepsAbsolutePath(
		repoRoot,
		recordSessionId,
	);
...
export async function persistRecordedSteps(
	...
	const relativePath = await writeRecordedSteps(
		repoRoot,
		recordSessionId,
		steps,
	);
...
export async function persistSessionState(
	...
	const nextArtifacts = Array.from(
		new Set([...existing.artifacts, ...artifacts]),
	);
	...
	if (sessionRecord) {
		const nextRecord: PersistedSessionRecord = {
			...
			artifacts: Array.from(
				new Set([...sessionRecord.artifacts, relativePath]),
			),
			updatedAt: new Date().toISOString(),
		};
