...
| RN readiness doctor | experimental, fixture-backed | `pnpm run validate:react-native-readiness` | `scripts/showcase/react-native-readiness.ts` | device inventory via MCP `list_devices` + Metro target check + readiness contract + stable selector contract |
| RN evidence pack | experimental, fixture-backed | `pnpm run validate:react-native-evidence-pack` | `scripts/showcase/react-native-evidence-pack.ts` | RN readiness artifact + JS signal summary + native evidence references + proof boundary |
| RN one-command lane | experimental, fixture-backed | `pnpm run verify:react-native-change` | `scripts/showcase/react-native-one-command.ts` | readiness -> evidence pack -> compact result orchestration |
...
- `react-native-readiness/v1`：检查 device、Metro、JS debug target、readiness contract、stable selectors。
- `react-native-evidence-pack/v1`：把 readiness、JS signal summary、native evidence reference、failure summary 合成 review artifact。
- `react-native-one-command/v1`：提供 `pnpm run verify:react-native-change` 的 developer-facing orchestration。
...
1. **RN evidence pack 还没有真正采集 live JS signals。**
   当前 pack 可以接收 console/network summary，但默认 fixture 仍是 unavailable。它证明了 artifact shape，不证明真实 RN runtime 诊断能力。

2. **RN one-command 还没有接上 live mobile-change verification runner。**
   现在 one-command 只编排 readiness 和 evidence pack。它能减少“前置条件不明”的摩擦，但还没有形成完整 `change -> live RN app verification -> intake -> PR evidence`。

3. **稳定 selector 只是声明式输入，没有扫描 app/source。**
   readiness doctor 目前检查 `M2E_RN_STABLE_SELECTORS` 是否存在，不会验证这些 selector 是否真的在 RN source 或 device UI tree 中可见。

4. **Expo / bare RN / dev-client / release APK 的差异还没有建模。**
   当前 profile 只说 deterministic app entry via Expo/Metro or native app launch，但没有把这些模式拆成不同 readiness strategy。
...
### P0：把 RN one-command 接到 live verification/intake

目标：让 `pnpm run verify:react-native-change` 不只停在 readiness/evidence pack，而是可选地进入 live verification runner。
...
### P0：增加 RN selector/source audit

目标：从“用户声明 stableSelectors”升级为“仓库可检查 selector 是否存在”。
...
### P1：区分 Expo Go / Expo dev-client / bare RN / release artifact

目标：把 app entry 模式显性化，避免“RN 支持”变成隐藏前置条件。
...
### P1：RN JS runtime failure taxonomy

建议基于 console/network snapshot 形成 RN-specific reason codes：
