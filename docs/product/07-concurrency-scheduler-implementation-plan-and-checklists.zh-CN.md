...
- [x] 同一 `deviceId` 在任意时刻只允许一个 session 执行设备操作
- [x] 不同 `deviceId` 的 session 可并发执行
- [x] `start_session` 获取租约、`end_session` 释放租约
- [x] `run_flow`/`perform_action_with_evidence` 等 session-bound 工具进入统一执行守卫
- [x] 调度事件写入 session timeline，保留审计可追溯性

### 暂不做（本轮排除）

- [x] 不引入分布式队列/外部 DB（Redis/Kafka 等）
- [x] 不在 `adapter-maestro` 内实现调度逻辑
...
- [x] 新增 `core` 租约存储与租约管理
- [x] `start_session` 与 `end_session` 接入 acquire/release
...
- [x] 增加统一 `withSessionExecution` 包装器
- [ ] `run_flow` 从“直接转发”升级为“加载 session + 执行守卫 + 调用 adapter”
- [x] 将 session-bound 工具统一走守卫
- [x] stale lease 回收（TTL + heartbeat）

完成定义：并发压力下无设备抢占冲突，session 工具执行顺序可解释。

## Phase C（观测收口 + 双设备 sync 预留，2 周）

- [x] timeline 增加 lease/scheduler 事件
- [x] 报告补充 queue wait / lease conflict 等指标
- [x] 预留 `coordinationKey`/`barrierId`（内部字段，不破坏 MCP 外部 contract）
...
- [x] 调度状态归属 `packages/core`
- [x] 工具编排归属 `packages/mcp-server`
- [x] 设备执行归属 `packages/adapter-maestro`
- [x] 不破坏现有 `Session` / `ToolResult` 主干结构
- [x] 保持 `end_session` 幂等语义
