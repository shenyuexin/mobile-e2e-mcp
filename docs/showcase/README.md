...
- AUT readiness contract scaffold: `configs/readiness/mobile-change.android.json`
- Repo-owned demo app readiness contract: `configs/readiness/demo-android-app.android.json`
- Repo-owned app success candidate: `docs/showcase/evidence/mobile-change-repo-app-success-candidate/candidate.md`
- Mobile change CI/PR evidence: `docs/showcase/evidence/mobile-change-ci-pr-evidence/pr-summary.md`
- Mobile change failure memory: `docs/showcase/evidence/mobile-change-failure-memory/remediation.md`
- React Native selector audit: `docs/showcase/evidence/react-native-selector-audit/audit.md`
- React Native runtime contract: `docs/showcase/evidence/react-native-runtime-contract/contract.md`
- React Native readiness: `docs/showcase/evidence/react-native-readiness/report.md`
- React Native evidence pack: `docs/showcase/evidence/react-native-evidence-pack/evidence-pack.md`
- React Native one-command lane: `docs/showcase/evidence/react-native-one-command/result.md`
- React Native live bridge command: `pnpm run verify:react-native-change -- --live-bridge --contract=configs/readiness/demo-android-app.android.json --output-dir=output/showcase/react-native-one-command-cli/<run-id>`
- React Native live success candidate: `docs/showcase/evidence/react-native-live-success-candidate/candidate.md`
- React Native failure taxonomy: `docs/showcase/evidence/react-native-failure-taxonomy/taxonomy.md`
...
- AUT readiness contract proof (`pnpm run test:mobile-change-readiness-contract` + `pnpm run validate:mobile-change-readiness-contract`): verifies a scaffolded app-under-test readiness contract has app id, deterministic entry, reset semantics, and deterministic ready-state signals before live proof relies on it. Visual-only contracts are valid only as weak guidance and cannot promote strong proof.
- Repo-owned app success candidate proof (`pnpm run test:mobile-change-repo-app-success-candidate` + `pnpm run validate:mobile-change-repo-app-success-candidate`): verifies the deterministic readiness contract and committed artifact-availability claim for the repo-owned demo Android app, then records that the current no-device environment is blocked before live success. It must not be described as successful app evidence until a connected device or explicitly labeled emulator run passes intake.
- Mobile change CI/PR evidence proof (`pnpm run test:mobile-change-ci-pr-evidence` + `pnpm run validate:mobile-change-ci-pr-evidence`): verifies the compact CI artifact and PR summary preserve proof labels. Blocked/no-device evidence maps to neutral CI status; only promoted physical/emulator evidence can map to success.
- Mobile change failure memory proof (`pnpm run test:mobile-change-failure-memory` + `pnpm run validate:mobile-change-failure-memory`): groups existing failure packets and blocked candidates into deterministic patterns. Repeated readiness failures route to readiness-contract repair, environment blockers route to the device doctor, and weak evidence routes to inspect-first collection.
- React Native selector audit proof (`pnpm run test:react-native-selector-audit` + `pnpm run validate:react-native-selector-audit`): verifies declared RN stable selectors are present as source-level `testID`/accessibility literals. It is static source evidence and does not prove runtime UI visibility.
- React Native runtime contract proof (`pnpm run test:react-native-runtime-contract` + `pnpm run validate:react-native-runtime-contract`): verifies Expo Go, Expo dev-client, bare debug, and bare release prerequisites are explicit. Release mode requires an app artifact and must not be blocked by missing Metro or JS debug target.
- React Native evidence pack proof (`pnpm run test:react-native-evidence-pack` + `pnpm run validate:react-native-evidence-pack`): verifies RN readiness, Metro JS/network summaries, native evidence references, and RN failure taxonomy stay reviewable without promoting Metro-only evidence to live success.
- React Native one-command proof (`pnpm run test:react-native-one-command` + `pnpm run validate:react-native-one-command`): verifies `react-native-one-command/v2` preserves proof labels and exposes an optional live bridge into mobile-change verification/intake. The committed fixture remains blocked before live. Ad-hoc live bridge attempts can use `--live-bridge`, `--contract`, and `--output-dir` without changing committed fixture evidence.
- React Native live success candidate proof (`pnpm run test:react-native-live-success-candidate` + `pnpm run validate:react-native-live-success-candidate`): verifies RN live success promotion is allowed only when the one-command live bridge completed with physical/emulator proof and verification plus intake evidence. The committed candidate is blocked before RN live success because no live bridge ran in the current environment.
- React Native failure taxonomy proof (`pnpm run test:react-native-failure-taxonomy` + `pnpm run validate:react-native-failure-taxonomy`): maps RN readiness blockers and JS/network signals to stable `RN_*` reason codes with bounded next actions. It is evidence grouping, not autonomous remediation.
