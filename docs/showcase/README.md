...
- Repo-owned app success candidate proof (`pnpm run test:mobile-change-repo-app-success-candidate` + `pnpm run validate:mobile-change-repo-app-success-candidate`): verifies the repo-owned demo Android APK and deterministic readiness contract are present, then records that the current no-device environment is blocked before live success. It must not be described as successful app evidence until a connected device or explicitly labeled emulator run passes intake.
